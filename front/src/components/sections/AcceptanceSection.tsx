...
export function AcceptanceSection() {
  const sectionData = useDocumentStore(s => s.sections?.acceptance) as Record<string, any> | undefined
  ...
  if (!hasContent) {
    return (
      <div>
        ...
        <p style={{ color: '#999' }}>인수 기준이 아직 정의되지 않았습니다. 채팅에서 "Acceptance Criteria 작성해줘"라고 요청하세요.</p>
      </div>
    )
  }
