...
export function AssumptionsSection() {
  ...
  const handleAddPresetGroup = useCallback((presetIndex: number) => {
    const preset = ASSUMPTIONS_PRESET_GROUPS[presetIndex]
    if (!preset) return
    ...
    setDocument({ sections: { ...sections, assumptions: updated } } as any)
    doSave(() => saveUserInput(docId, 'sections.assumptions.groups', updatedGroups))
    ...
    return (
      <div>
        <h2 style={headingStyle}>
          ...
          <SectionGuideButton sectionKey="assumptions" />
        </h2>
        <div style={emptyContainer}>
          <p style={emptyMainText}>
            가정 및 리스크가 아직 정의되지 않았습니다.
    ...
    <div ref={containerRef} style={{ position: 'relative', display: 'inline-block' }}>
      ...
      {open && (
        <div style={presetDropdown}>
          {ASSUMPTIONS_PRESET_GROUPS.map((group, idx) => (
            <div
              key={idx}
