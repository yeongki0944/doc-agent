...
export function SuccessCriteriaSection() {
  const sectionData = useDocumentStore(s => s.sections?.success_criteria) as Record<string, any> | undefined
  ...
  if (!hasContent) {
    return (
      <div>
        ...
        <p style={{ color: '#999' }}>성공 기준이 아직 정의되지 않았습니다. 채팅에서 "Success Criteria 작성해줘"라고 요청하세요.</p>
      </div>
    )
  }
